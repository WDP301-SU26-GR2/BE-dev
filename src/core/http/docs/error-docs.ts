...
export const ERROR_HINTS: Record<string, string> = {
  ...
  'Error.AnnotationTaskBindingInvalid': 'taskId must belong to the annotated Page/Region/Task target',
  ...
  'Error.StageNotFound': 'stageId does not exist or does not belong to the requested chapter',
  'Error.StageAccessDenied': 'caller is outside the stage chapter scope or is not the owning mangaka for a mutation',
  'Error.StageRequired':
    'a stageId is required when the chapter has ProductionStage records; legacy chapters remain compatible',
  'Error.StageLocked': 'the selected stage is not ACTIVE yet; complete prior stages first',
  'Error.StageNotActive': 'only the current ACTIVE stage can be completed or receive output confirmation',
  'Error.StageHasOpenTasks': 'approve or cancel every open task in the stage before confirming output or completing it',
  'Error.StageOutputNotReady': 'confirm an output for every stage page before completing the stage',
  'Error.StagePageNotFound': 'the requested page has no immutable input snapshot for this stage',
  'Error.StageOutputInvalid':
    'outputs must cover the exact stage page set once, with exactly one of fileKey or reuseInput',
  'Error.ProductionPageSetLocked': 'pages can only be added or deleted while the first production stage is ACTIVE',
