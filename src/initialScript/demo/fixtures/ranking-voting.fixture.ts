...
export const seedRankingsAndVoting = async (
  context: DemoContext,
  series: SeriesSeed[],
  monthlySeries: SeriesSeed[]
) => {
  const creator = requiredAccount(context.accounts, 'admin.hikari')
  ...
  await seedMonthlyRankingsAndVoting(context, monthlySeries)
}

const seedMonthlyRankingsAndVoting = async (context: DemoContext, monthlySeries: SeriesSeed[]) => {
  const creator = requiredAccount(context.accounts, 'admin.hikari')
  const atRiskIds = new Set(monthlySeries.slice(-3).map((row) => row.id))
  const monthlyPeriodMs = 28 * DAY
  let previous = new Map<string, number>()
  ...
  for (let periodIndex = 0; periodIndex < DEMO_HISTORY_DAYS; periodIndex += 1) {
    const period = await context.prisma.surveyPeriod.create({
      data: {
        ...
        eligibleSeriesIds: monthlySeries.map((row) => row.id),
        issueNumber: 500 + periodIndex,
        reflectedIssueNumber: 492 + periodIndex,
        startDate: new Date(
          context.now.getTime() - (DEMO_ITERATIONS + DEMO_HISTORY_DAYS - periodIndex + 1) * monthlyPeriodMs
        ),
        endDate: new Date(
          context.now.getTime() - (DEMO_ITERATIONS + DEMO_HISTORY_DAYS - periodIndex) * monthlyPeriodMs
        ),
        status: SurveyStatus.REFLECTED
      }
    })
    const healthy = monthlySeries.filter((row) => !atRiskIds.has(row.id))
    const risky = monthlySeries.filter((row) => atRiskIds.has(row.id))
    const ordered = [
      ...healthy.slice(periodIndex % healthy.length),
      ...healthy.slice(0, periodIndex % healthy.length),
      ...risky
    ...
    await context.prisma.rankingRecord.createMany({
      data: ordered.map((row, index) => {
        ...
        const old = previous.get(row.id)
        const consecutive = atRiskIds.has(row.id) ? periodIndex + 1 : 0
        const voteCount = 1820 - index * 135 + periodIndex * 9
        return {
          surveyPeriodId: period.id,
          seriesId: row.id,
          rankPosition: rank,
          voteCount,
          normalizedScore: Number((voteCount / 1820).toFixed(4)),
          previousRank: old ?? null,
          rankChange: old ? old - rank : null,
          isAtRisk: atRiskIds.has(row.id),
          riskLevel: !atRiskIds.has(row.id)
  ...
  for (let index = 0; index < DEMO_ITERATIONS; index += 1) {
    const period = await context.prisma.surveyPeriod.create({
      data: {
        ...
        reflectedIssueNumber: 592 + index,
        startDate: new Date(context.now.getTime() - (DEMO_ITERATIONS - index + 1) * monthlyPeriodMs),
        endDate: new Date(context.now.getTime() - (DEMO_ITERATIONS - index) * monthlyPeriodMs),
        status: SurveyStatus.CLOSED
      }
    })
    await context.prisma.surveyData.create({
      data: {
        surveyPeriodId: period.id,
        importedBy: creator.id,
        surveyDate: new Date(context.now.getTime() - (DEMO_ITERATIONS - index) * monthlyPeriodMs),
        entries: monthlySeries.map((row, seriesIndex) => ({
          seriesId: row.id,
          voteCount: 55 + index * 6 + seriesIndex * 9
        }))
      }
    })
    for (let voteIndex = 0; voteIndex < 5; voteIndex += 1) {
      await context.prisma.readerVote.create({
        data: {
          surveyPeriodId: period.id,
          seriesIds: [
            monthlySeries[(voteIndex + index) % monthlySeries.length].id,
            monthlySeries[(voteIndex + index + 1) % monthlySeries.length].id
          ],
          identityHash: hash(`demo-monthly-voter-${index}-${voteIndex}`),
          publicationType: PublicationType.MONTHLY,
          authMethod: ReaderAuthMethod.EMAIL_OTP,
          ipHash: hash(`198.51.100.${20 + voteIndex}`),
          captchaScore: voteIndex === 4 ? 0.55 : 0.91,
          voteWeight: voteIndex === 4 ? 0.5 : 1,
          isFlagged: voteIndex === 4
        }
  ...
  await context.prisma.surveyPeriod.create({
    data: {
      ...
      eligibleSeriesIds: monthlySeries.map((row) => row.id),
      issueNumber: 700,
      reflectedIssueNumber: 700,
      startDate: new Date(context.now.getTime() - DAY),
      endDate: new Date(context.now.getTime() + 27 * DAY),
      status: SurveyStatus.OPEN
    }
  })
}
