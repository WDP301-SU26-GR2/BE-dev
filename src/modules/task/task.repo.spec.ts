...
describe('TaskRepository response enrichment', () => {
  it('listTasks only hydrates assistant and does not look up embedded TaskVersion submitters', async () => {
    ...
    const response = toTaskListItem(result[0])

    expect(prisma.user.findMany).toHaveBeenCalledTimes(1)
    expect(prisma.user.findMany.mock.calls[0][0].where.id.in).toEqual(['assistant'])
    expect(response.assistant?.displayName).toBe('Assistant')
    expect('versions' in response).toBe(false)
  })
