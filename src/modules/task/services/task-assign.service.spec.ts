...
import { AssetNotFoundException, AssistantNotHiredException, NotSeriesOwnerException, TaskNotReassignableException } from '../errors/task.errors'
...
describe('TaskAssignService', () => {
  ...
  const service = new TaskAssignService(repo as never, studio as never, storage as never, taskState as never, notification as never)
  beforeEach(() => jest.clearAllMocks())

  it('rejects non-owner → 403', async () => {
    repo.findPageWithOwner.mockResolvedValue(PAGE)
    await expect(
      service.create('OTHER', { pageId: ID, assistantId: ID, taskType: 'BACKGROUND', priority: 0, assetIds: [] } as never)
  ...
  it('rejects missing asset → 422', async () => {
    ...
    await expect(
      service.create('m', { pageId: ID, assistantId: ID, taskType: 'BACKGROUND', priority: 0, assetIds: ['k1'] } as never)
  ...
  it('creates task ASSIGNED + notifies', async () => {
    ...
    await service.create('m', { pageId: ID, assistantId: ID, taskType: 'BACKGROUND', priority: 0, assetIds: [] } as never)
  ...
  it('reassign rejects non-ON_HOLD task → 409', async () => {
    ...
    repo.findPageWithOwner.mockResolvedValue(PAGE)
    await expect(service.reassign('m', ID, { assistantId: ID } as never)).rejects.toBe(TaskNotReassignableException)
  })
})